...
driver.get('https://www.dhlottery.co.kr/store.do?method=topStore&pageGubun=SP2000')
    ...
    if len(list_items) >= 2:
        ...
        for j in range(1, len(list_items)):
            ...
            if len(res_data['addresses']) < 1:
                cur.execute("""INSERT INTO sp2000 (shop, location, ranking, round) VALUES ("%s", "%s", "%d", "%d")""" % (shop, location, 1, i))
            else:
                ...
                cur.execute("""INSERT INTO sp2000 (shop, location, ranking, round, x, y) VALUES ("%s", "%s", "%d", "%d", "%f", "%f")""" % (shop, location, 1, i, float(res_data['addresses'][0]['x']), float(res_data['addresses'][0]['y'])))
                conn.commit()
    ...
    if len(second_list_items) >= 2:
        ...
        for j in range(1, len(second_list_items)):
            ...
            if len(res_data['addresses']) < 1:
                ...
                cur.execute("""INSERT INTO sp2000 (shop, location, ranking, round, x, y) VALUES ("%s", "%s", "%d", "%d", "%f", "%f")""" % (shop, location, 2, i, float(res_data['addresses'][0]['x']), float(res_data['addresses'][0]['y'])))
                conn.commit()
